...
export default function Home() {
  ...
  const API_URL = url + "/discover/tv?&" + API_KEY
  const fetchAllMovies = async () => {
      const response = await fetch(API_URL)
      const movie = await response.json();
      setAllTV(movie.results)
  }
  useEffect(() => {
    fetchAllMovies()
  },[])
  return (
    <div className="page-bg overflow-auto hide-scroll px-2 ">
      <div className="flex items-center">
        <SearchInput setFiltered={setFiltered} allShows={allTV}/>
        <Filter allShows = {allTV} setFiltered={setFiltered} activeGenre={activeGenre} setActiveGenre={setActiveGenre}/>
      </div>
      <motion.div 
